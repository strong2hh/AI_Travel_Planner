...
//             const token = localStorage.getItem('employee_token');
//             if (!token) {
//                 this.showError('身份认证失败，无法使用语音识别！');
//                 return;
//             }
//
//             const headers = {
//                 // 注意：由于 body 是 FormData，Content-Type 浏览器会自动设置，
//                 // 但我们需要手动设置 token 头部
//                 'token': token
//             };
//
...
//         daySelect.innerHTML = '';
//
//         // ★★★ 核心修改：从 this.scheduleData.days 数组中获取天数 ★★★
...
//             this.createDaySchedulePanel(1, null); // 创建一个空的日程面板
//             this.switchDay('day1');
//             return;
//         }
//
...
//             const dayNumber = dayData.day; // 使用 DayDTO 中的 day 字段
...
//     }
//
//     // 创建日程面板 (修改为接受活动列表作为参数)
//     createDaySchedulePanel(dayNumber, activities) {
//
//         const scheduleContent = document.querySelector('.schedule-content');
//         if (!scheduleContent) return;
//
//         // ★★★ 修正开始：添加缺失的 daySchedule 元素创建代码 ★★★
//         const daySchedule = document.createElement('div');
//         daySchedule.id = `day${dayNumber}-schedule`; // 确保ID可以被 switchDay 引用
...
//
//         // ★★★ 核心修改：使用传入的 activities 数组来渲染 ★★★
//         const dayActivities = activities;
//
//         if (dayActivities && dayActivities.length > 0) {
//             // 使用 updateScheduleUI 中的逻辑来渲染活动项和箭头
//             // 由于 updateScheduleUI 负责渲染所有天，这里先留空，主要依靠 updateScheduleUI
...
//     }
//
//     updateMapMarkers(dayKey) { // 接收 'day1', 'day2' 等 key
//         // 清除之前的地图标记
//         this.clearDayMarkers();
//
//         // ★★★ 核心修改：从 days 数组中找到对应天数的数据 ★★★
...
//             // ... (getCoordinates 和 addMarker 的逻辑不变)
...
//                 // AMap.Marker 对象通常有一个 setMap(null) 方法来从地图上移除自己
//                 if (marker && marker.setMap) {
//                     marker.setMap(null);
//                 }
//             });
//             this.dayMarkers = []; // 清空数组
//         }
//     }
//
//     // 显示/隐藏加载状态
...
//
//         const token = localStorage.getItem('employee_token'); // ★★★ 新增：获取令牌 ★★★
//         if (!token) {
...
//
//         try {
//             const response = await fetch(`${BACKEND_BASE_URL}/api/schedule/getSchedule`, {
//                 method: 'GET',
//                 headers: {
//                     'token': token // ★★★ 修正：添加令牌到请求头 ★★★
//                 }
//             });
//
//             if (!response.ok) {
...
//             }
//
//             const result = await response.json(); // 假设返回 Result<ScheduleDTO>
//
//             if (result.code !== 200) {
//                 // 如果状态码不是 200，则肯定是错误
...
//
//             if (!result.data) {
//                 // code = 200，但 data 为 null，视为无历史数据
//                 this.scheduleData = {};
//                 console.log('用户暂无历史行程记录 (后端返回 data: null)');
//                 // 这里不需要抛出错误，继续执行即可
//             } else {
//                 // code = 200 且 data 有效，正常处理
...
//
//             // 确保 switchDay 被调用，如果 loadScheduleData 中已经调用 generateDayOptions
//             // 且 generateDayOptions 内部调用了 switchDay('day1')，这里可以省略。
//             // 为了安全，我们再次调用 switchDay 来确保 UI 和地图更新。
//             if (this.scheduleData && this.scheduleData.days && this.scheduleData.days.length > 0) {
//                 this.switchDay('day1');
//             } else {
//                 this.updateMapMarkers('day1'); // 清除地图标记
//             }
//         }
//     }
//
...
//                 if (title) {
//                     title.innerHTML = `<span id="schedule-theme-title">${this.scheduleData.theme || '行程安排'}</span> - 第${dayObject.day}天`;
//                 }
//
//                 const scheduleList = scheduleElement.querySelector('.schedule-list');
//                 if (scheduleList) {
//                     scheduleList.innerHTML = ''; // 清空现有内容
//
//                     const dayActivities = dayObject.activities || [];
//
//                     // 添加新的行程项目
...
//         window.mapApp = app;
//
//         // 【关键修改：使用 Promise 链启动 ScheduleManager】
//         app.init()
//             .then(() => {
//                 console.log('✅ 地图初始化流程完毕，开始初始化行程管理器');
//
//                 // ★★★ 步骤 1: 创建 ScheduleManager 实例 (局部变量) ★★★
//                 const scheduleManager = new ScheduleManager();
//
//                 // 步骤 2: 确保路径规划器初始化
//                 scheduleManager.initializeRoutePlanner();
//
//                 // 步骤 3: 将实例挂载到全局 window 对象
